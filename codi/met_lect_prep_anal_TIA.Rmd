---
title: "Meta_Analisi.Associations of macrovascular complications(Transient Ischaemic Attack-TIA) with death in patients with diabetes hospitalized for Coronavirus disease(COVID-19). \\pkg{foo}"
...
base_dades<-readxl::read_excel(conductor,sheet = "base_dades_TIA")
...
ROR_covid1<-base_dades%>%filter(COUNTRY=="UK_MOD1"       | COUNTRY=="FRANCE_MOD1"     | COUNTRY=="BCN_MOD1" )%>%
  select(COUNTRY2,LOG_OR,SE_OR)
ROR_covid2<-base_dades%>%filter(COUNTRY=="UK_MOD2"       | COUNTRY=="FRANCE_MOD2"     | COUNTRY=="BCN_MOD2" )%>%
  select(COUNTRY2,LOG_OR,SE_OR)
ROR_covid3<-base_dades%>%filter(COUNTRY=="UK_MOD3"       | COUNTRY=="FRANCE_MOD3"     | COUNTRY=="BCN_MOD3" )%>%
  select(COUNTRY2,LOG_OR,SE_OR)

ROR_covid1_uci<-base_dades%>%filter(COUNTRY=="UK_MOD1_uci"       | COUNTRY=="FRANCE_MOD1_uci"    )%>%
  select(COUNTRY2,LOG_OR,SE_OR)
ROR_covid2_uci<-base_dades%>%filter(COUNTRY=="UK_MOD2_uci"       | COUNTRY=="FRANCE_MOD2_uci"    )%>%
  select(COUNTRY2,LOG_OR,SE_OR)
ROR_covid3_uci<-base_dades%>%filter(COUNTRY=="UK_MOD3_uci"       | COUNTRY=="FRANCE_MOD3_uci"    )%>%
  select(COUNTRY2,LOG_OR,SE_OR)
...
```
## 3.1  	Meta-Analysis of Death/Uci/Imv~ Macrovascular disease -TIA- (unadjusted).
...
## Meta_Analisi Associations of macrovascular complications -TIA- with DEATH in patients with diabetes hospitalized for Coronavirus disease(COVID-19).Unadjusted model
...
## Meta_Analisi Associations of macrovascular complications -TIA- with UCI in patients with diabetes hospitalized for Coronavirus disease(COVID-19).Unadjusted model
...
## 3.2  	Meta-Analysis of Death/Uci/Uci_Death/Imv ~ Macrovascular -TIA- disease + Age + Sex.
...
## Meta_Analisi Associations of macrovascular complications -TIA- with DEATH in patients with diabetes hospitalized for Coronavirus disease(COVID-19).Ajusted model(Age + Sex).
* In this  case, there is  not variability between the 3 studies, so we will accept the average effect of the 3 studies (Common effect model).
...
## Meta_Analisi Associations of macrovascular complications -TIA- with UCI in patients with diabetes hospitalized for Coronavirus disease(COVID-19).Ajusted model(Age + Sex).
...
## 3.3  	Meta-Analysis of Death/Uci/Imv ~ Macrovascular disease -TIA- + Age + Sex + Hypertension + Type of Diabetes + Microvascular disease.
...
## Meta_Analisi Associations of macrovascular complications-TIA- with DEATH in patients with diabetes hospitalized for Coronavirus disease(COVID-19).Ajusted model(Age + Sex + Hypertension + Type of Diabetes + Microvascular disease).
* In this  case, there is  not variability between the 3 studies, so we will accept the average effect of the 3 studies (Common effect model).
...
## Meta_Analisi Associations of macrovascular complications-TIA- with UCI in patients with diabetes hospitalized for Coronavirus disease(COVID-19).Ajusted model(Age + Sex + Hypertension + Type of Diabetes + Microvascular disease).
* In this  case, there is  not variability between the 2 studies, so we will accept the average effect of the 2 studies (Common effect model).
...
readxl::read_excel(conductor_codis2,col_types = "text")%>%dplyr::select(cod,Descripcio,agr,agr0,agr2)%>% filter(agr=="TIA")%>%dplyr::select(cod,agr)%>% knitr::kable(caption="ICD10 TIA(TRANSIENT ISCHAEMIC ATTACK) Codes ")
